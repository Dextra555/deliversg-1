                    ...
                    Store.findOne({ _id: order.store_id }).then((store) => {
                        Order_payment.findOne({ _id: order.order_payment_id }).then((order_payment) => {
                            ...
                            if (order_status == ORDER_STATE.STORE_ACCEPTED) {
                                ...
                                response_data.json({
                                    success: false,
                                    error_code: CARD_ERROR_CODE.INVALID_PAYMENT_TOKEN,
                                    error_message: "Invalid order_status"
                                });
                            }
...
exports.capture_payment = function (store, order, request_data, order_payment, response_data) {
    ...
    if (order_payment.is_payment_mode_cash == false && order_payment.capture_amount > 0) {
        ...
        Payment_gateway.findOne({ _id: order_payment.payment_id }).then((payment_gateway) => {
            console.log('------------capture call----'+payment_gateway.name)
            if (payment_gateway.name == 'Stripe') {

                console.log('----'+Math.round(order_payment.card_payment * 100));
                                                ...
                                                order.save().then(() => {
                                                    ...
                                                    Order_payment.findOne({ _id: order.order_payment_id }).then((order_payment) => {
                                                        if (order_payment) {
                                                            Payment_gateway.findOne({ _id: order_payment.payment_id }).then((payment_gateway) => {
                                                                if (payment_gateway) {
                                                                    if ((order_status != ORDER_STATE.WAITING_FOR_ACCEPT_STORE && order_status != ORDER_STATE.ORDER_READY && payment_gateway.name == 'Stripe') || payment_gateway.name == 'Paystack') {
                                                                        ////console.log('----------is call------')
                                                                        utils.refund_payment_for_selected_payment_gateway(order_payment, function (response) { });
                                                                    }
                                                                    else if ((order_status >= ORDER_STATE.ORDER_READY && payment_gateway.name == 'Stripe') || payment_gateway.name == 'Paystack')
                                                                    {
                                                                        utils.cancel_payment_for_selected_payment_gateway(order_payment, function (response) { });
                                                                    }
                                                                }
