...
exports.reassign_accept_request = function (provider , request_data, response_data) {
...
exports.reassign_request_status_admin = function (request_data, response_data){
            ...
            Provider.findOne({_id: request_data_body.provider_id}).then((provider) => {
                if (provider)
                {
                    ...
                    else
                    {
                        ...
                        if (delivery_status == ORDER_STATE.DELIVERY_MAN_COMING || delivery_status == ORDER_STATE.DELIVERY_MAN_ARRIVED || delivery_status == ORDER_STATE.DELIVERY_MAN_PICKED_ORDER || delivery_status == ORDER_STATE.DELIVERY_MAN_STARTED_DELIVERY) 
                        {
                            var store = {}
                            var request_id = request_data_body.request_id;
                            var provider_id = request_data_body.provider_id;
                            
                            Request.findOne({ _id: request_id, provider_id: {$ne: request_data_body.provider_id} }).then((request) => {
                                console.log("request");
                                console.log(request);
                                if (request) 
                                {
                                        ...
                                        if (order) 
                                        {
                                            ...
                                            if (order.delivery_type == DELIVERY_TYPE.COURIER) 
                                            {
                                                ...
                                                Order_payment.findOne({ _id: order.order_payment_id }).then((order_payment) => {
                                                    if (order_payment) 
                                                    {
                                                        exports.reassign_accept_request(provider, request_data, function (return_data) {
...
exports.getRequestIdFromOrderId = function (request_data, response_data) {
    Order.findOne({_id: request_data.body.order_id}).then((order_detail) => {
        response_data.json({
            request_id : order_detail.request_id
        });
...
exports.findNearestProviderListsAdmin= function (request_data, response_data) {
    let falseObj= {
        status:false
    }
    Order.findOne({_id: request_data.body.order_id}).then((order_detail) => {
...
exports.findNearestProviderWithoutPushData = function (request, response_data) {
    Order.findOne({_id: request.orders[0].order_id}).then((order_detail) => {
        if (order_detail)
        {
                Provider.findOne({_id: request.current_provider}).then((provider) => {
...
exports.assign_request_status_admin = function (request_data, response_data){
    utils.check_request_params(request_data.body, [{name: 'delivery_status'}], function (response) {
        if (response.success) {
            console.log(request_data.body);
            var request_data_body = request_data.body;
                                            ...
                                            else 
                                            {
                                                ...
                                                Order_payment.findOne({ _id: order.order_payment_id }).then((order_payment) => {
                                                    ...
                                                    if (order_payment) {
                                                        ...
                                                        capture_payment(store, order, request_data, order_payment, function (capture_payment_response) {
                                                            console.log("Payment Response : "+capture_payment_response);
...
exports.get_rider_based_on_vehicle = function (request_data, response_data) 
{
    let provider_list = [];
    if(request_data.body.vehicle_id == null)
    {
        response_data.json({
            success: false,
        ...
        Provider.find({}).then((providers) => {
            ...
            providers.forEach(provider => {
                Provider_vehicle.findOne({ _id: provider.selected_vehicle_id }).then((provider_vehicles) => {
                    
                    if(provider_vehicles!=null)
                    {
                        var admin_vehicle_id = provider_vehicles.admin_vehicle_id;

                        if(admin_vehicle_id == request_data.body.vehicle_id)
                        {
                            // console.log("Selected User " + provider.first_name+ " " + provider.last_name);

                            var provider_data = {
                                vehicles:provider.vehicle_ids,
                                active_vehicle:provider.selected_vehicle_id,
                                provider_name:provider.first_name + " " + provider.last_name, 
                                provider_email:provider.email, 
                                provider_id:provider._id,
                                provider_token:provider.server_token
                            }

                            provider_list.push(provider_data);
                        }
...
exports.get_vehicles_lists_all = function (request_data, response_data) {
...
exports.get_vehicles_list = function (request_data, response_data) {
    utils.check_unique_details(request_data, [], function (response) {
        ...
        var delivery_type = DELIVERY_TYPE.STORE;
        console.log(DELIVERY_TYPE.STORE0)
        if (request_data_body.delivery_type != null) {
            delivery_type = request_data_body.delivery_type;
        }
        var group = {
        ...
        var delivery_type_query = { $match: { delivery_type: { $eq: delivery_type } } };
        Service.aggregate([condition, delivery_type_query, type_condition, lookup, unwind, group]).then((services) => {
             console.log(Service.aggregate([condition, delivery_type_query, type_condition, lookup, unwind, group]))
            if (services.length > 0) {
...
exports.check_accept_request = function (provider , request_data, response_data) {
    var request_data_body = request_data.body;
    var request_id = request_data_body.request_id;
    var delivery_status = Number(request_data_body.delivery_status);
    console.log(request_data_body)
...
exports.check_assigned_request_status = function (request_data, response_data){
    // var request_data_body = request_data.body;
    // var provider_id = request_data_body.provider_id;
    // var server_token = request_data_body.server_token;
    // var request_status = request_data_body.request_status;
    // var delivery_status = request_data_body.delivery_status;

    // response_data.json({provider_id:provider_id, server_token:server_token, request_status:request_status, delivery_status:delivery_status});
                                            ...
                                            if (order.delivery_type == DELIVERY_TYPE.COURIER) {
                                                Order_payment.findOne({ _id: order.order_payment_id }).then((order_payment) => {
                                                    if (order_payment) {
                                                        capture_payment(store, order, request_data, order_payment, function (capture_payment_response) {
                                                            if (capture_payment_response.success) {
                                                                exports.check_accept_request(provider, request_data, function (return_data) {
                                                                    response_data.json(return_data);
                                                                });
                                                            } else {
                                                                response_data.json({ success: false });
